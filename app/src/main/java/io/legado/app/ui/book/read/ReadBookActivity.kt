...
class ReadBookActivity : BaseReadBookActivity(),
    ...
    AiBgMusicSettingsDialog.Callback,
    ...
    private var readAloudFloatCoverAnimator: ObjectAnimator? = null
    ...
    private val aiBgMusicDirLauncher =
        registerForActivityResult(OpenDocumentTree()) { uri ->
            ...
            AiBgMusic.musicDir = uri.toString()
            toastOnUi("背景音乐目录已选择")
        }
...
//            }
    ...
    override fun contentLoadFinish() {
        if (intent.getBooleanExtra("readAloud", false)) {
            ...
            TtsServerDbBridge.ensureRunning(this@ReadBookActivity)
            ReadBook.readAloud()
        }
        loadStates = true
        updateGeneratedAudiobookFloatButtonState()
    ...
    @SuppressLint("ClickableViewAccessibility")
    private fun bindAiBgMusicFloatDrag() {
        val button = binding.aiBgmFloatButton
        var downRawX = 0f
        var downRawY = 0f
        var startX = 0f
        ...
        button.setOnTouchListener { view, event ->
            when (event.actionMasked) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    if (!dragged && kotlin.math.hypot(dx.toDouble(), dy.toDouble()) > touchSlop) {
                        dragged = true
                    }
                    if (dragged) {
                        val maxX = (binding.rootView.width - view.width).coerceAtLeast(0).toFloat()
    ...
    private fun openReadAloudFloatPlayer() {
        generatedAudiobookFloatActive = false
        if (!BaseReadAloudService.isRun) {
            onClickReadAloud()
        }
    ...
    private fun toggleReadAloudPlaybackOnly() {
        when {
    ...
    private fun updateAiBgMusicFloatButtonState(playing: Boolean = BaseReadAloudService.isPlay()) {
        ...
        updateFloatingCoverRotation(playing)
        updateGeneratedAudiobookFloatButtonState()
    }
    ...
    private fun updateAiBgMusicFloatButtonAppearance() {
        ...
        binding.readAloudFloatPlayButton.apply {
            val playIconColor = Color.rgb(0xFF, 0xF8, 0xEA)
            iconTint = ColorStateList.valueOf(playIconColor)
            setTextColor(playIconColor)
            rippleColor = ColorStateList.valueOf(ripple)
            setBackgroundResource(R.drawable.bg_read_aloud_play_button)
        }
    ...
    private fun updateFloatingPlayerText() {
        val bookName = ReadBook.book?.name.orEmpty().ifBlank { "正在朗读" }
        val chapterTitle = ReadBook.curTextChapter?.title.orEmpty().ifBlank { "当前章节" }
        binding.readAloudFloatTitle.text = if (generatedAudiobookFloatActive) "整章音频" else bookName
        binding.readAloudFloatSubtitle.text = chapterTitle
        val playing = BaseReadAloudService.isPlay()
        val icon = if (playing) R.drawable.ic_pause_filled else R.drawable.ic_play_filled
        binding.readAloudFloatPlayButton.setIconResource(icon)
        binding.readAloudBottomChip.setIconResource(icon)
        binding.readAloudBottomChip.text = if (playing) "暂停播放" else "继续播放"
    }
    ...
    private fun positionGeneratedAudiobookBubble() {
        ...
        positionFloatingPanelNearAnchor(
            if (generatedAudiobookFloatActive) bubble else anchor,
            binding.readAloudFloatPanel,
            0
        )
    ...
    private fun positionFloatingPanelNearAnchor(anchor: View, panel: View, bottomGapDp: Int) {
        ...
        if (panel.width <= 0 || panel.height <= 0 || binding.rootView.width <= 0) {
            panel.post { positionGeneratedAudiobookBubble() }
            return
        }
        val rootWidth = binding.rootView.width.coerceAtLeast(1)
        val rootHeight = binding.rootView.height.coerceAtLeast(1)
        val fallbackX = rootWidth - panel.width - 18.dpToPx()
        val fallbackY = rootHeight - panel.height - (92 + bottomGapDp).dpToPx()
        val targetX = if (anchor.width > 0 && anchor.visibility == View.VISIBLE) {
            anchor.x + anchor.width - panel.width
        } else {
            fallbackX.toFloat()
        }.coerceIn(0f, (rootWidth - panel.width).coerceAtLeast(0).toFloat())
        val targetY = if (anchor.height > 0 && anchor.visibility == View.VISIBLE) {
            anchor.y + (anchor.height - panel.height) / 2f
        } else {
            fallbackY.toFloat()
        }.coerceIn(0f, (rootHeight - panel.height).coerceAtLeast(0).toFloat())
        panel.x = targetX
        panel.y = targetY
    }

    private fun positionBottomChip(chip: View, bottomDp: Int) {
        if (chip.visibility != View.VISIBLE) return
        if (chip.width <= 0 || chip.height <= 0 || binding.rootView.width <= 0) {
            chip.post { positionGeneratedAudiobookBubble() }
            return
        }
        val rootWidth = binding.rootView.width.coerceAtLeast(1)
        val rootHeight = binding.rootView.height.coerceAtLeast(1)
    ...
    private fun updateFloatingCoverRotation(playing: Boolean) {
        readAloudFloatCoverAnimator = updateCoverRotationAnimator(
            ...
            playing && BaseReadAloudService.isRun
        )
    }

    private fun updateCoverRotationAnimator(
        view: View,
    ...
    private fun showAiBgMusicMenu(anchor: View) {
        aiBgMusicMenuPopup?.let {
            if (it.isShowing) {
                it.dismiss()
                aiBgMusicMenuPopup = null
                return
            }
        }
        val items = listOf(
            "🎛️ 设置" to { showDialogFragment<AiBgMusicSettingsDialog>() },
            "🎚️ 频率" to { showAiBgMusicFrequencyDialog() },
            "📜 列表" to { showAiBgMusicPlaylistDialog() },
            "🧠 分析" to { showAiBgMusicAnalysisDialog() },
            "🔄 重析" to { confirmAiBgMusicReanalyze() },
        )
        ...
        val popup = PopupWindow(
            ...
            setOnDismissListener { aiBgMusicMenuPopup = null }
        }

        items.forEachIndexed { index, item ->
            val child = buildAiBgMusicMenuItem(item.first) {
                popup.dismiss()
                item.second.invoke()
            }
            root.addView(child)
            if (index != items.lastIndex) {
                root.addView(View(this).apply {
                    layoutParams = LinearLayout.LayoutParams(1, 7.dpToPx())
                })
            }
        }
        root.measure(
            View.MeasureSpec.makeMeasureSpec(binding.rootView.width, View.MeasureSpec.AT_MOST),
            View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED)
        )
        val rootWidth = binding.rootView.width.coerceAtLeast(1)
        val rootHeight = binding.rootView.height.coerceAtLeast(1)
        val popupWidth = root.measuredWidth.coerceAtLeast(96.dpToPx())
        val popupHeight = root.measuredHeight
        val buttonCenterX = anchor.x + anchor.width / 2f
        val buttonCenterY = anchor.y + anchor.height / 2f
        val gap = 12.dpToPx()
        val popupX = if (buttonCenterX > rootWidth * 0.5f) {
            (anchor.x - popupWidth - gap).toInt()
        } else {
            (anchor.x + anchor.width + gap).toInt()
        }.coerceIn(8.dpToPx(), (rootWidth - popupWidth - 8.dpToPx()).coerceAtLeast(8.dpToPx()))
        val popupY = (buttonCenterY - popupHeight / 2f)
            .toInt()
            .coerceIn(8.dpToPx(), (rootHeight - popupHeight - 8.dpToPx()).coerceAtLeast(8.dpToPx()))
        aiBgMusicMenuPopup = popup
        popup.showAtLocation(binding.rootView, Gravity.NO_GRAVITY, popupX, popupY)
        root.alpha = 0f
        root.scaleX = 0.92f
        root.scaleY = 0.92f
        root.animate().alpha(1f).scaleX(1f).scaleY(1f).setDuration(140).start()
    }
    ...
    private fun styleAiBgmMenuButton(view: android.widget.TextView) {
        view.setBackgroundResource(R.drawable.bg_ai_bgm_menu_item)
        view.setTextColor(resolveThemeColor(com.google.android.material.R.attr.colorOnSurface))
        view.elevation = 10.dpToPx().toFloat()
        view.alpha = 0.98f
        view.setPadding(14.dpToPx(), 8.dpToPx(), 14.dpToPx(), 8.dpToPx())
    }

    private fun buildAiBgMusicMenuItem(text: String, action: () -> Unit): TextView {
    ...
    private fun showAiBgMusicFrequencyDialog() {
        val root = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
        ...
        val frequencySlider = Slider(this).apply {
            valueFrom = 0f
            valueTo = 2f
            stepSize = 1f
            value = AiBgMusic.frequency.toFloat()
        }
        val scenesText = TextView(this).apply {
            textSize = 15f
        }
        val scenesSlider = Slider(this).apply {
            valueFrom = 1f
            valueTo = 10f
            stepSize = 1f
            value = AiBgMusic.scenesPerMusic.toFloat()
        }
        fun updateTexts() {
            frequencyText.text = when (frequencySlider.value.toInt()) {
                AiBgMusic.FREQUENCY_BOOK -> "整本书一种基调音乐反复播放"
                AiBgMusic.FREQUENCY_CHAPTER -> "不同章节不同背景音乐"
                else -> "按场景切换背景音乐"
            }
            val sceneCount = scenesSlider.value.toInt()
            scenesText.text = if (sceneCount == 1) {
                "每 1 个场景切换一次音乐"
            } else {
                "每 $sceneCount 个场景共用一种音乐"
            }
            scenesText.visibility = if (frequencySlider.value.toInt() == AiBgMusic.FREQUENCY_SCENE) View.VISIBLE else View.GONE
            scenesSlider.visibility = scenesText.visibility
        }
        frequencySlider.addOnChangeListener { _, _, _ -> updateTexts() }
        scenesSlider.addOnChangeListener { _, _, _ -> updateTexts() }
        root.addView(frequencyText)
        root.addView(frequencySlider)
        root.addView(scenesText)
        root.addView(scenesSlider)
        updateTexts()
        AlertDialog.Builder(this)
            .setTitle("音乐切换频率")
            .setView(root)
            .setPositiveButton("保存") { dialog, _ ->
                val oldFrequency = AiBgMusic.frequency
                val oldScenes = AiBgMusic.scenesPerMusic
                AiBgMusic.frequency = frequencySlider.value.toInt()
                AiBgMusic.scenesPerMusic = scenesSlider.value.toInt()
                val changed = oldFrequency != AiBgMusic.frequency || oldScenes != AiBgMusic.scenesPerMusic
                if (changed) {
                    ReadBook.book?.let {
                    ...
                    toastOnUi("音乐切换频率已更新，正在重新分析播放列表")
                } else {
                    toastOnUi("音乐切换频率已保存")
                }
                dialog.dismiss()
    ...
    private fun showAiBgMusicPlaylistDialog() {
        ReadBook.book?.let {
    ...
    private fun confirmAiBgMusicReanalyze() {
        AlertDialog.Builder(this)
            .setTitle("重新分析生成播放列表")
            .setMessage("重新分析会按当前播放模式、提示词、模型和音乐目录覆盖当前播放列表，是否继续？")
            .setPositiveButton("重新分析") { _, _ ->
                ReadBook.book?.let {
                    AiBgMusic.analyzeRange(
                        it,
                        ReadBook.durChapterIndex,
                    ...
                    toastOnUi("AI 还在分析，请你耐心等待，心急吃不了热豆腐")
                }
            }
            .setNegativeButton(android.R.string.cancel, null)
            .show()

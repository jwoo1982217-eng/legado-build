...
object BookController {
    /**
     * TTS Server 整章直通：获取当前正在阅读/朗读的章节全文。
     */
    fun getCurrentChapterForTTS(): io.legado.app.api.ReturnData {
        val returnData = io.legado.app.api.ReturnData()
        try {
            val book = io.legado.app.model.ReadBook.book
                ?: return returnData.setErrorMsg("当前没有打开的书籍")

            val textChapter = io.legado.app.model.ReadBook.curTextChapter
                ?: return returnData.setErrorMsg("当前章节尚未加载")

            val chapter = textChapter.chapter
            val text = textChapter.getContent().trim()

            if (text.isBlank()) {
                return returnData.setErrorMsg("当前章节正文为空")
            }

            val textHash = java.security.MessageDigest.getInstance("MD5")
            ...
            val data = linkedMapOf<String, Any?>(
                ...
                "contentSource" to "ReadBook.curTextChapter",
                "updatedAt" to System.currentTimeMillis()
            )

            return returnData.setData(data)
        } catch (e: Exception) {
            return returnData.setErrorMsg(e.toString())
        }
    }
